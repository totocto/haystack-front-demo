...
export default {
  ...
  methods: {
    ...
    async onExternalRefClick(refId) {
      ...
      const { query } = this.$route
      this.$router.push({ hash: refId, query: { a: query.a, q: newApiFilter, d: query.d, l: query.l } }).catch(() => {})
    },
    async onGraphClick(pointName) {
      ...
      if (!this.isPointFromSource(pointName, colorEntities)) {
        ...
        this.$store.commit('SET_FILTER_API', { filterApi: newApiFilter })
        this.$router.push({ query: { a: query.a, q: newApiFilter, d: query.d, l: query.l } }).catch(() => {})
      } else {
